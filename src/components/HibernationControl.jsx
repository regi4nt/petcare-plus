...
export const HibernationControlModal = ({
  ...
  return createPortal(
    // Wrapper dengan data-dark agar CSS dark mode berlaku di dalam portal
    <div
    ...
    >
      {/* Card — rounded-[32px] & shadow konsisten dengan modal lain */}
      <div className="w-full max-w-md bg-white rounded-[32px] shadow-2xl flex flex-col overflow-hidden" style={{ maxHeight: '90vh' }}>

        {/* ── Header — gradient + dekorasi circle (pola AddPetModal) ── */}
        <div className="bg-gradient-to-br from-indigo-600 to-violet-600 px-7 pt-7 pb-6 rounded-t-[32px] relative overflow-hidden shrink-0">
          ...
          <div className="flex justify-between items-start relative">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 bg-white/20 rounded-2xl flex items-center justify-center shrink-0">
                <BatteryLow size={20} className="text-white" />
              </div>
              <div>
                <p className="text-indigo-200 text-xs font-bold uppercase tracking-widest mb-0.5">Kontrol Perangkat</p>
                <h3 className="text-2xl font-black text-white leading-tight">Hibernasi</h3>
                <p className="text-indigo-200 text-xs font-semibold mt-0.5">{petName} · {deviceId}</p>
              </div>
            </div>
            <button
              onClick={onClose}
          ...
          {/* Status indicator */}
          <div className={`mt-4 flex items-center gap-2 px-3 py-2 rounded-2xl text-xs font-bold transition-all ${
            isHibernating
        ...
        {/* ── Tabs ─────────────────────────────────────────────── */}
        <div className="flex border-b border-slate-100 px-6 pt-1 bg-white shrink-0">
          {[
            ...
            <div className="px-6 py-5 space-y-5">
                ...
                {selectedPreset === 0 && (
                  <div className="mt-3 flex items-center gap-2">
                    <input
                      ...
                      className="flex-1 border-2 border-slate-200 rounded-2xl px-4 py-2.5 text-sm font-semibold focus:border-indigo-400 focus:outline-none transition-all bg-white text-slate-800"
                    />
              ...
              <div className="space-y-3">
                ...
                >
                  ...
                  <ChevronRight size={16} className="opacity-60 shrink-0 group-hover:translate-x-1 transition-transform" />
                </button>
          ...
          {activeTab === 'riwayat' && (
            <div className="px-6 py-5">
              <div className="flex items-center justify-between mb-4">
                ...
                >
                  {histLoading ? <Loader2 size={12} className="animate-spin" /> : <History size={12} />}
                  Refresh
                </button>
              </div>
